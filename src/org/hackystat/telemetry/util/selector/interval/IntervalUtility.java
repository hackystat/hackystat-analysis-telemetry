package org.hackystat.telemetry.util.selector.interval;
...
/**
 * Provides supports for constantant and method for interval selector.
 *
 * @author Hongbing Kou
 * @version $Id: IntervalUtility.java,v 1.1.1.1 2005/10/20 23:56:40 johnson Exp $
 */
class IntervalUtility {
  ...
  private IntervalUtility() {
    ...
    for (int i = 1; i <= 31; i++) {
      String dayValue = (i < 10) ? "0" + String.valueOf(i) : String.valueOf(i);
      this.dayOptions.put(dayValue, dayValue);
    }
    
    // Month options
    //this.monthOptions = new TreeMap<String, String>(new MonthNameComparator());
    ...
    for (int i = 0; i < monthStrings.length - 1; i++) {
      String monthValue = (i >= 0 && i < 10) ? "0" + String.valueOf(i) : String.valueOf(i);
  ...
  private void fillWeekOptions() {
    ...
    if (weekSizeString == null) {
      Day today = Day.getInstance();
      start = new Week(today.inc(-1 * 52 * 7));
    }
    else { // Week is in dd-MMM-yyyy format
      start = getWeek(weekSizeString);      
    }
   ...
   * The week string will be something like "11-Jan-2004 to 17-Jan-2004", or 
   * its locale-related variant.
  ...
  public Week getWeek(String weekString) {
    // First, let's try using the 'native' locale.
    ...
    // Current locale bombed, so now try with an English-specific locale.
   ...
   */
  public TreeMap getMonthOptions() {
  ...
  private class WeekComparator implements Comparator<String> {
    /**
  ...
  private static class IntervalUtilityHolder {
    ...
    static final IntervalUtility THE_INSTANCE = new IntervalUtility();
  }
