...
public class TestUnitTestChartRestApi extends TelemetryTestHelper {
  ...
  @Test public void testUnitTestChartFailureCount() throws Exception {
    ...
    // Check that the first three are zero and the last is null.
    assertEquals("Checking point 1 is zero", "0", points.get(0).getValue());
    assertEquals("Checking point 2 is zero", "0", points.get(1).getValue());
    assertEquals("Checking point 3 is zero", "0", points.get(2).getValue());
